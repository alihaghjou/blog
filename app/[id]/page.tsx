  ...
  return (
    <main className="m-auto w-3/4 min-h-screen">
      <article className="border-b p-6 flex flex-col gap-3">
        ...
        <h2 className="text-sm text-gray-600 py-2">{new Date(post.inserted_at).toDateString()}</h2>
        <p className="indent-2 leading-6 text-justify" style={{textJustify: "inter-word"}}>{post.content}</p>
      </article>
    </main>
  );
}
